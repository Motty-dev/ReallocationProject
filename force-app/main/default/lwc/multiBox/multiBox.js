...
export default class MultiBox extends LightningElement {
    ...
    @track displayedValue = '';

    @api
    set options(value) {
        // Convert the passed array into the internal rows format
        this.rows = value.map(option => ({
            label: option.label,
            value: option.value,
            selected: false,
            classes: 'dropdown-item' // this can be used to control visibility with CSS
        }));
    }

    get options() {
        // Extract the values from the rows for external use
        return this.rows.filter(row => row.selected).map(row => row.value);
    }

    // Event handlers for selection, filtering, and applying the selection would go here

    applySelection() {
        // Custom event with selected options
